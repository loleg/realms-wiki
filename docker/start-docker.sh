...
sudo docker run --name realms-wiki -p 5000:5000 -d \
 --link postgresql:db -v /srv/realms/wiki:/opt/wiki -v /srv/realms/etc:/etc/realms-wiki/ \
 realms-wiki-git

echo "sudo docker exec -it realms-wiki /usr/bin/tail -f /var/log/realms-wiki/realms-wiki.log"
# ./lib/python2.7/site-packages/realms/static/css/style.css
